...
import { Save, ArrowLeft, Eye, Code, AlertCircle, Trash2, X } from 'lucide-vue-next'
...
// State
const pageTitle = ref(`Edit Site: ${props.site.name}`)
...
const form = useForm<FormData>({
  ...
  subdomain: props.site.subdomain || '', // Use existing subdomain or empty string
  description: props.site.description || '',
  theme: props.site.theme || '',
  theme_name: props.site.theme_name || '',
  status: props.site.status,
  menuItems: typeof props.site.menuItems === 'string' 
    ? props.site.menuItems 
    : JSON.stringify(props.site.menuItems || [], null, 2)
})
...
const deleteMenuItem = (index: number) => {
  try {
    ...
    jsonError.value = `Cannot delete menu item: ${(e as Error).message}`
  }
}

const submit = () => {
  console.log('Submit function called for edit')
  ...
  form.put(`/admin/sites/${props.site.id}`, {
    onStart: () => {
...
const deleteSite = () => {
  if (confirm(`Are you sure you want to delete the site "${props.site.name}"? This action cannot be undone.`)) {
    form.delete(`/admin/sites/${props.site.id}`, {
      onSuccess: () => {
        // Will redirect via backend
      }
    })
  }
}
...
// Lifecycle
onMounted(() => {
  // Initialize JSON format if needed
  if (form.menuItems && isValidJson.value) {
    formatJson()
  }
})
</script>
  ...
  <DefaultLayout>
    ...
    <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex gap-2">
            <button
              @click="deleteSite"
              type="button"
              class="inline-flex items-center justify-center rounded-md bg-red-500 py-2 px-4 text-center font-medium text-white hover:bg-red-600"
            >
      ...
      <form @submit.prevent="submit" class="p-6.5">
            ...
            <div v-if="jsonMode" class="border border-stroke rounded p-4 dark:border-strokedark">
              ...
              <div v-if="isValidJson && parsedMenuItems.length > 0" class="space-y-3">
                <div
                  ...
                  :key="index"
                  class="border border-gray-200 rounded p-3 dark:border-gray-600 relative"
                >
                  <button
                  ...
                  <div class="font-medium text-black dark:text-white pr-8">
                    {{ item.title }} ({{ item.col }})
                  </div>
        ...
        <!-- Site Info -->
        <div class="mb-6 p-4 bg-gray-50 rounded dark:bg-meta-4">
          <h4 class="text-sm font-medium text-black dark:text-white mb-2">Site Information</h4>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <span class="text-gray-600 dark:text-gray-300">Site ID:</span>
              <span class="ml-2 font-medium text-black dark:text-white">{{ site.id }}</span>
            </div>
            <div>
              <span class="text-gray-600 dark:text-gray-300">Created:</span>
              <span class="ml-2 font-medium text-black dark:text-white">{{ new Date(site.created_at).toLocaleDateString() }}</span>
            </div>
            <div>
              <span class="text-gray-600 dark:text-gray-300">Last Updated:</span>
              <span class="ml-2 font-medium text-black dark:text-white">{{ new Date(site.updated_at).toLocaleDateString() }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="flex justify-end gap-4">
          ...
          >
            ...
            {{ form.processing ? 'Updating...' : 'Update Site' }}
          </button>
        </div>
      </form>
    </div>
    <!-- Edit Site Form Section End -->
  </DefaultLayout>
</template>