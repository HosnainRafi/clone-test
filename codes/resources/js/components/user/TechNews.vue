...
// Computed properties for dynamic data processing
const displayNews = computed(() => {
  let news = [...props.techNewsData.news];
  
  // Filter active news
  news = news.filter(item => item.isActive);
  
  // Sort news based on sortBy and sortOrder
  news.sort((a, b) => {
    let comparison = 0;
    
    switch (props.techNewsData.sortBy) {
      case 'date':
      ...
      case 'views':
        ...
        break;
      case 'trending':
        comparison = Number(a.trending) - Number(b.trending);
        break;
    }
    
    return props.techNewsData.sortOrder === 'desc' ? -comparison : comparison;
  });
  
  // Limit to maxNews
  return news.slice(0, props.techNewsData.maxNews);
});
...
const getCategoryColor = (category: string): string => {
  const colors: Record<string, string> = {
    ...
    'Technology': 'bg-gray-100 text-gray-800 hover:bg-gray-200',
    'AI/ML': 'bg-violet-100 text-violet-800 hover:bg-violet-200',
...
<template>
  ...
  >
    ...
    <!-- Grid View (when carousel is disabled) -->
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <Card 
        v-for="news in displayNews" 
        :key="news.id"
        class="group h-full bg-white hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border-2 border-gray-100 hover:border-blue-200"
      >
        <!-- News Image/Header -->
        <div class="relative overflow-hidden rounded-t-lg">
          <div 
