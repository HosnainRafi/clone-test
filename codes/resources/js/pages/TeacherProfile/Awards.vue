...
const form = ref<{ awards: Award[] }>({
    awards: props.teacher.awards.length > 0 ? [...props.teacher.awards] : [],
});
...
const saveAwards = async () => {
    ...
    try {
        const response = await fetch('/admin/teacher/profile/awards', {
            method: 'POST',
            headers: {
        ...
        toast.error('Error saving awards');
    } finally {
        saving.value = false;
    }
};
    ...
    <TeacherProfileLayout :teacher="{ id: props.teacher.id, name: props.teacher.name }">
        <div class="shadow-default rounded-sm border border-stroke bg-white dark:border-strokedark dark:bg-boxdark">
            <div class="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                <h3 class="font-semibold text-black dark:text-white">Awards & Honors</h3>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Highlight your achievements and recognitions.</p>
            </div>

            <div class="p-6.5">
                <form @submit.prevent="saveAwards" class="space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        >
                            <div class="mb-4 flex items-center justify-between">
                                <h4 class="font-medium text-black dark:text-white">Award #{{ index + 1 }}</h4>
                                <button
                                    type="button"
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-black dark:text-white">
                                        Award Title <span class="text-meta-1">*</span>
                                    </label>
                                    <input
                                        v-model="award.title"
                                        type="text"
                                ...
                                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                    <div>
                                        <label class="mb-2 block text-sm font-medium text-black dark:text-white">Event/Organization</label>
                                        <input
                                            v-model="award.event"
                                            type="text"
                                            placeholder="e.g., IEEE Conference 2023"
                                            class="w-full rounded border-[1.5px] border-stroke bg-white px-4 py-2.5 transition outline-none focus:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                                        />
                                    </div>
                                    <div>
                        ...
                        <div
                            v-if="form.awards.length === 0"
                            class="rounded-lg border-2 border-dashed border-stroke bg-gray-50 p-12 text-center dark:border-strokedark dark:bg-meta-4"
                        >
                            <p class="text-gray-600 dark:text-gray-400">No awards added yet.</p>
                            <p class="mt-1 text-sm text-gray-500">Click "Add Award" to get started.</p>
                        </div>
                    </div>
